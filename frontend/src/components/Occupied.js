...
const Occupied = ({ racks = [] }) => { 
  ...
  // Function to check if a spot should be visible (not hidden)
  const shouldShowSpot = (column, row, stack) => {
            ...
            {Array.from({ length: 33 }, (_, rowIndex) => rowIndex + 1).map(row => (
              <div key={`${column}-${row}`} className="rack-row grid grid-cols-5 w-full mr-2 object-contain">
                {Array.from({ length: 5 }, (_, stackIndex) => stackIndex + 1).map(stack => {
                  ...
                  // Set the opacity of racks that are not in the fullSpots array to 0
                  const opacityClass = shouldShowSpot(column, row, stack) ? '' : 'opacity-0';
